...
contract ConsensusContract {
  ...
  mapping(string => Consensus) patients; //maps a patient_id to a consensus struct that represent the evolution of their consensus.
  function createconsensus(string memory _patient_id, uint _keyhash ) onlyOwner public {// create a new consensus linked to a patient on the chain.
